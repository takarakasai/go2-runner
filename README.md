...
| 指令 vx | Grip2 v̄x 体系（オドメトリ） | 体系 v̄y | ω̄z | 世界系 y 変位 |
|---|---|---|---|---|
| 0.6 | 0.636（0.538） | +0.002 | −0.031 | −1.46 m |
| 0.8 | **0.831**（0.731） | +0.006 | −0.043 | −2.41 m |
| 1.0 | **1.023**（0.917） | +0.012 | −0.053 | −3.36 m |

いずれも転倒せず、傾きは |roll| 1.9° / |pitch| 1.7° 以内。その場旋回
（wz 0.8）は 12 s で変位 0.02 m = **本当にその場**、横移動（vy 0.2）は
12 s で 2.29 m = 0.19 m/s。

**「横流れ」は横移動ではなくヨー流れだった**（2026-09-20 に測り直し）。この表の
旧版は世界系の平均速度と y 変位で書かれており、方策がヨーに流れて円弧を描くと
前進が過小・横が過大に出る。体座標系で測ると **v̄y は最大でも +0.012 m/s** で、
残っているのは ω̄z −0.03〜−0.05 rad/s のヨーバイアス。前進も 0.957 →
**1.023 m/s（102 %）**に上がる。旧版の「Grip2 は Rival より横流れが半分」も
実体は「ヨー流れが半分」で、Rival 側は測り直していない。
